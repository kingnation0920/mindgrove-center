...
export default async function BlogPostPage({ params }: PostPageProps) {
  ...
  return (
    <article className="bg-white min-h-screen py-12 md:py-20">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <header className="border-b border-gray-200 pb-8 mb-10">
          ...
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-950 leading-snug tracking-tight mb-6">
            {post.title}
          </h1>

          <div className="flex flex-wrap items-center gap-y-2 gap-x-6 text-xs sm:text-sm text-gray-500">
        ...
        {/* Post Body (Preserved HTML from original clickn board) */}
        <div
          className="board_contents prose prose-lg max-w-none text-gray-800 leading-relaxed font-normal"
          dangerouslySetInnerHTML={{ __html: post.contentHtml }}
        />

        {/* Call To Action Banner */}
        <div className="mt-16 p-8 bg-gradient-to-r from-orange-50/70 to-amber-50/70 border border-orange-200/70 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-6 shadow-sm">
        ...
        <nav className="mt-12 pt-8 border-t border-gray-200 grid grid-cols-1 sm:grid-cols-2 gap-4">
          {prevPost ? (
            ...
            >
              <span className="text-xs text-gray-400 flex items-center gap-1 mb-1">
                <ArrowLeft className="w-3 h-3 group-hover:-translate-x-0.5 transition-transform" /> 이전 글
              </span>
              <span className="text-sm font-semibold text-gray-800 group-hover:text-brand-orange line-clamp-1 transition-colors">
                {prevPost.title}
              </span>
            </Link>
          ) : <div />}
          ...
          {nextPost ? (
            ...
            >
              <span className="text-xs text-gray-400 flex items-center gap-1 mb-1">
                다음 글 <ArrowRight className="w-3 h-3 group-hover:translate-x-0.5 transition-transform" />
              </span>
              <span className="text-sm font-semibold text-gray-800 group-hover:text-brand-orange line-clamp-1 transition-colors">
                {nextPost.title}
              </span>
